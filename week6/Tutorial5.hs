...
prob4 = entries "517462893684973125239581476476829351891356742352147968965718234728634519143295687"
...
printSudoku = putStrLn.intercalate "\n--- + --- + ---\n".map ((intercalate "\n".map (intercalate " | ")).map (f 3)).(f 3).(f 9)
    where
    f n [] = []
    f n l = take n l : f n (drop n l)